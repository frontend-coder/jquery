...
$(".drezden ul").css({'padding-top' : '20px',
...
$(".drezden__1 ul").css({'padding-top' : '20px',
	'list-style-type':'none',
	'color' : 'red'});
...
$(".drezden__1 ul li:has(a)").css({'color' : '#585858', 'text-decoration':'underline'});
$(".drezden__1 ul li a").css({'text-decoration':'line-through'});

$(".drezden__2 ul").css({'margin-top':'20px'});
$(".drezden__2 ul li:contains(первый)").css({'text-decoration':'line-through', 'color':'red'});
$(".drezden__2 ul li:contains(шестой)").css({'text-decoration':'overline', 'color':'red'});


$(".drezden__3").css({'margin-top':'20px'});
$(".drezden__3 ul li:first").css({ 'list-style-type':'none',
	'text-decoration':'overline',
	'color':'red'});
$(".drezden__3 ul li:last").css({ 'list-style-type':'none',
...
$(".drezden__4 h2").css({'margin-top':'20px','text-align': 'center', 'margin-bottom':'20px'});


$(".drezden__4 ul li:last").css({ 'list-style-type':'none',
	'text-decoration':'underline',
	'color':'red'});
...
$('#vivod_text').css('cursor','pointer').hide(2000);
$('#vivod_text').show(5000);


$('#vivod_text4').css('cursor','pointer').hide(2000).show(5000);

$(".click").on('click', function() {
	let vivodText = $('#vivod_text4').width();
	let vivodHeight = $('#vivod_text4').height();
	alert('длина текста: ' + vivodText + 'px' + "vivodHeight: " +vivodHeight+'px' );
});

//Добавить текст на jQuery, Методы
const textInSide = $('.jmi_data').text();
//alert(textInSide);
$("#jmi_data_exet").text('Новое значение переменной составляет '+textInSide);
$(".jmi").css({'margin-top' : '2rem', 'font-size':'24px', 'color' : 'red', 'text-decoration' :'overline'});
$(".jmi").hide('2000').show('4000'); // цепнпя функция

// this_button
// alert($('.this_button').width());
$('.this_button').css({'margin-top' : '120px', 'display' : 'inline-block', 'border' : '1px solid red', 'padding': '20px 30px' });
$('.this_button').width('290px');


const thisButton = $('.that_button').width();
$('.that_button2').width(280).css({'text-align':'center', 'display': 'inline-block', 'border' : '1px solid red'});
...
$('#geremu .geremu_item').html('<p>Этo новый контект, который будет добавляться из jquery</p>');

$('#geremu .geremu_item_two').html('<p>Этo новый контект, который будет добавляться из jquery </p>' + oneDelegam);


$("#gerundiy_text").css({'margin-bottom' : '50px', 'margin-top' : '50px', 'display' : 'inline-block', 'border' : '1px solid red', 'padding': '20px 30px' });


$("#gerundiy_text").before('<p>Текст перед контейнером </p>');
$("#gerundiy_text").after('<p>Текст после контейнером </p>');

$("#gerundiy_text").prepend('<p>1.Текст в начале наполнениия контейнером </p>');
$("#gerundiy_text").append('<p>2.Текст в конце наполнениия контейнером </p>');



$('#attribution .attribution_link').attr('title', 'Это название титла ссылки');

$('#attribution .attribution_link').attr('alt', 'Это описание ссылки');

$('#attribution .attribution_pic').attr('alt', 'Это я работаю с описанием картинки');
...
$('#box_box_twoo ul li').fadeOut(1300).fadeIn(3000).fadeTo(2000, .1);
...
$('#box_box_three ul li').animate({
	'font-size': '23px',
	'padding' : '10px 20px'
}, 6000, console.log('процедура завершена') );
...
$('.parent_wrap_items').parent().css({'border':'1px solid red', 'padding':'20px'});
$('.parent_wrap_items').parent().parent().css({'border':'1px solid blue', 'padding':'20px'});
$('#parent').children().children().children().children().css({'border':'1px solid olive', 'padding':'20px'});


$('#closest').css({'border-top':'1px solid blue', 'margin-top':'20px', 'padding-top':'20px'});
$('#closest h2').css({'margin-bottom':'20px', 'padding-top':'20px'});

$('#closest .parent_wrap_item p:last').closest('div').css({'color': 'red', 'margin-bottom':'20px', 'padding-top':'20px'});


$(".parent_wrap_item p").parents().css({'border': '1px solid yellow'});
...
$('#attribution .attribution_pic').css({'margin-top': '40px', 'display':'inline-block', 'max-width': '100%'});
...
$('.find_over_wrap_items ul').find('li:last').css({'border': '1px solid red'});

$('.find_over_wrap_items ul').find('li:eq(1)').css({'margin':'10px', 'padding':'10px', 'border': '1px solid orange'});



$("#basic_prev").prev().css({'margin':'10px', 'padding':'10px', 'border': '1px solid blue'});

$("#basic_prev").prev().prev().css({'margin':'10px', 'padding':'10px', 'border': '10px solid grey'});


$("#basic_prev").prev().prev().prev().css({'margin':'10px', 'padding':'10px', 'border': '10px solid orange'});
